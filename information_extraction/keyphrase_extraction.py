...
def _get_ngrams(sequence, n_gram_length):
    return list(
        zip(
            *(
                sequence[index:]
                for index in range(n_gram_length)
            )
        )
    )
...
def get_chunks(
    ...
    for sentence in sentences:
        ...
        wtc_sents = [
            nltk.chunk.tree2conlltags(chunk) for chunk in chunks
        ]
        flattened_chunks = list(
            itertools.chain.from_iterable(wtc_sents)
        )
        valid_chunks_tagged = [
            (status, [wtc for wtc in chunk])
            for status, chunk in itertools.groupby(
                flattened_chunks,
                lambda word_pos_chunk: word_pos_chunk[2] != "0"
            )
        ]
        valid_chunks = [
            " ".join(
